...
exports.QueryString = (function () {
    ...
    QueryString.prototype.filter = function (urlString) {
        ...
        let query = url.search ? querystring.unescape(url.search.replace(/^\?/, "")) : "";
    ...
    /**
     * Attach the request observer to modify query parameters
     */
    QueryString.prototype.attachRequestObserver = function () {
        ...
        events.on("http-on-modify-request", function (event) {
            ...
            // Only modify urls for the top document - what appears in the URL bar
            // @todo - maybe make this a preference. User might want it to be on all urls
            let isTopDocument = utils.isChannelForTopDocument(channel);
            if (!isTopDocument) {
                console.warn("NOT TOP DOCUMENT: " + channel.URI.spec);
                dump(event.subject);
                return;
            }
            ...
            if (uri && uri.spec && -1 !== uri.spec.indexOf('?')) {
                ...
                if (filteredUrl !== originalUrl) {
                    ...
                    utils.redirectChannelToUrl(channel, filteredUrl);
                }
            }
        }, true);
    };
